...
type WalletKeyImport struct {
	// The daemon host to sync the wallet with
	DaemonHost string `json:"daemonHost,omitempty"`
	// The daemon port to sync the wallet with
	DaemonPort int32 `json:"daemonPort,omitempty"`
	// The filename to store/load the wallet from. Note that this is relative to where turtle-wallet-api was launched from - it is recommended you use an absolute path.
	Filename string `json:"filename,omitempty"`
	// The password used to open the wallet
	Password string `json:"password,omitempty"`
	// The block height to start scanning from
	ScanHeight int32 `json:"scanHeight,omitempty"`
	// 64 char hex private view key
	PrivateViewKey string `json:"privateViewKey,omitempty"`
	// 64 char hex private spend key
	PrivateSpendKey string `json:"privateSpendKey,omitempty"`
}
